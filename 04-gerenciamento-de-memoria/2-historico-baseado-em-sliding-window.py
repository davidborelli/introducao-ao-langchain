...
prompt = ChatPromptTemplate.from_messages([
  ("system", "You're a helpfull assistant that answer with a shot joke when possible."),
...
response1 = conversational_chain.invoke({"input": "My name is David. reply only with 'OK' and do not mention my name."}, config=config)
print("Assistant", response1.content)
print("-"*30)

response2 = conversational_chain.invoke({ "input": "Tell me a one-setence fun fact. Do not mention my name" }, config=config)
